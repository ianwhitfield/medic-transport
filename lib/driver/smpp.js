...
// TODO(iwhitfield): This probably isn't the right pattern.
var splitter = require('./smpp/smsglobal_splitter');
var addresser = require('./smpp/smsglobal_addresser');
...
exports.prototype = {
  ...
  send: function (_message, _callback) {
    ...
    var pdus_to_send = splitter.createPdus(_message.content);
    pdus_to_send.forEach(addresser.addressPdu.bind(this._addresser, _message.to));

    // TODO: There's some sort of send-multi that we might want to use here.
    session.submit_sm(pdus_to_send[0] /* !!! */, function(_recv_pdu) {
      if (_recv_pdu.command_status == 0) {
        // Message successfully sent
        console.log('Successfully sent');
        console.log(_recv_pdu.message_id);
      } else {
        console.log('Error status: ', _recv_pdu.command_status);
      }
    });
  ...
  register_receive_handler: function (_handler) {
    session.on('submit_sm', function(_pdu) {
      console.log('Received message: ', _pdu);
      var message = {};
      message.timestamp = '12345'; // TODO
      message.from = pdu.source_addr;
      message.content = pdu.short_message.filter.decode(pdu.short_message.type);

...
exports.prototype = {
  ...
  initialize: function (_options) {
    ...
    this._session.bind_transceiver({
      ...
      if (pdu.command_status == 0) {
        ...
        deferred.reject(
          new Error("Failed to bind, status: ", 
              pdu.command_status.toString(16)));
      }
    });
  ...
  send: function (_message, _callback) {
    ...
    this._promise = this._promise.then(function() {
      var pdus_to_send = that._splitter.prototype.createPdus(_message.content);
      pdus_to_send.forEach(
          that._addresser.prototype.addressPdu.bind(that._addresser, _message.to));

      // TODO: There's some sort of send-multi that we might want to use here.
      for (var i = 0; i < pdus_to_send.length; i++) {
        that._session.submit_sm(pdus_to_send[i], function(_recv_pdu) {
          if (_recv_pdu.command_status == 0) {
            // Message successfully sent
            console.log('Successfully sent');
            console.log(_recv_pdu.message_id);
          } else {
            console.log('Error while sending, status: ',
                _recv_pdu.command_status.toString(16));
          }
        });
      }
    }, function(err) {
      console.log('Error, send() called but session is not bound: ', err);
    });
  ...
  register_receive_handler: function (_handler) {
    this._session.on('deliver_sm', function(_pdu) {
  ...
  stop: function () {
    this._promise = this._promise.then(function() {
      ...
      console.log('Error, stop() called but session is not bound: ', err);
    });
